...
def sim_returns(starting_b_value = 0.015, low_bound_b = 0.02, upper_bound_b = 0.04, ltr_vol_std = 0.1, probability_of_default = 0.01, ltr_default_prob = 0.001 , starting_staking_rate = 0.015, r_t_sigma = 0.005 ,  sigma_market_time = 2 , max_prob = 0.6, sigma_b = 15 ):
    ### r_t needs to be normalized on yearly level, because we will have exponential growth!!!
    ### We normalize everything in order to set it up for yearly return (so, exponential growth):
    starting_b_value = np.log(1 + starting_b_value)
    low_bound_b = np.log(1 + low_bound_b)
    upper_bound_b = np.log(1 + upper_bound_b)
    starting_staking_rate = np.log(1 + starting_staking_rate)
    ...
    while t<T:
        ...
        returns[t] = returns[t-1] + returns[t-1] * r_t[t] * year - catastrophic_event((default_prob_summed[t])*year) * loss_share(max_prob)*returns[t-1]
        
        t+=1
    return(market_time,long_term_avg_stake,b_value,r_t,default_prob_summed,returns)
